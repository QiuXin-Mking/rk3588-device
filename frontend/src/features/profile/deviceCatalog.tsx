...
export const DEVICE_SUITES: DeviceSuite[] = [
	...
	},
	{
		...
		cameraKey: null,
		altConnected: (record) =>
			Boolean(record.gloveSides?.left || record.gloveSides?.right),
	},
];
...
export function isDeviceConnected(device: DeviceSuite, record: RecordStatus) {
	...
	if (device.cameraKey && record.cameras)
		return Boolean(record.cameras[device.cameraKey]);
	return false;
}
...
export function DataBadge({ label }: { label: string }) {
	...
	return (
		<Badge size={mode === "device" ? "device" : "default"} variant="outline">
			{label}
		</Badge>
	);
}
