				...
				<Card className="overflow-auto p-7">
					{mode === 'local' ? (
						...
						<>
							...
							<div className="my-5 grid grid-cols-2 gap-4">
								<label className={deviceLabelClass}><span>Endpoint</span><input className={deviceControlClass} placeholder="https://oss-cn-xxx.example.com" /></label>
								<label className={deviceLabelClass}><span>Bucket</span><input className={deviceControlClass} placeholder="sensorhub-dataset" /></label>
								...
								<label className={deviceLabelClass}><span>区域 Region</span><input className={deviceControlClass} placeholder="cn-east-3" /></label>
							</div>
							...
							<div className="mt-5 flex justify-end gap-3"><Button size="device" variant="secondary" onClick={test}>{testing ? '检查中…' : '测试配置'}</Button><Button size="device" onClick={() => notify(`${provider} 配置已保存在当前页面，待后端接入`)}>保存配置</Button></div>
						</>
					)}
				</Card>
      </div>
		...
		<div className="page detail-page">
			...
			<div className={cn('grid min-h-0 flex-1 gap-5 overflow-y-auto', mode === 'device' ? 'grid-cols-[minmax(300px,.72fr)_minmax(620px,1.55fr)]' : 'grid-cols-1 gap-3')}>
				...
				<Card className={cn('grid content-start overflow-auto', mode === 'device' ? 'gap-4 p-7' : 'gap-3 p-4')}>
					...
					<label className={controlLabelClass(mode)}><span>问题描述</span><textarea className={controlClass(mode)} rows={5} placeholder="请描述发生步骤、期望结果和实际结果" /></label>
					<label className={controlLabelClass(mode)}><span>联系方式（选填）</span><input className={controlClass(mode)} placeholder="手机或邮箱" /></label>
					<Button size={mode === 'device' ? 'device' : 'touch'} onClick={() => notify(`${kind}已保存在页面，提交接口待接入`)}><Send />提交反馈</Button>
				</Card>
			</div>
		</div>
	)
		...
		<div className="page detail-page">
			...
			<div className={cn('grid min-h-0 flex-1 gap-5 overflow-y-auto', mode === 'device' ? 'grid-cols-[minmax(300px,.72fr)_minmax(620px,1.55fr)]' : 'grid-cols-1 gap-3')}>
				<Card className={cn('overflow-auto', mode === 'device' ? 'p-7' : 'p-4')}>
					...
					<span className="eyebrow">WEARING GUIDE</span><h2 className={cn('mt-2 font-bold', mode === 'device' ? 'text-[length:var(--device-text-lg)]' : 'text-xl')}>{steps[step]}</h2>
				...
				<Card className={cn('overflow-auto', mode === 'device' ? 'p-7' : 'p-4')}>
					...
					<p className={cn('my-4 leading-relaxed', mode === 'device' ? 'text-[length:var(--device-text-xs)]' : 'text-sm')}>发现相机画面明显卡顿时，正确做法是？</p>
